...
System.out.println("Please enter the number of players.");        max = getInt("   It should be an integer value greater than or equal to 2.");
System.out.println("Constructing list of players");     players = new AList<Integer>();
        for(int i = 1; i <= max; i++) 
        	players.add(i);
...
for(int i = 1,j = startAt; i <= rhyme.getLength();i++,j++)
	 { f(j > players.getLength()) j = 1; System.out.println(rhyme.getEntry(i) + " : Player"+players.getEntry(j));  toBeRemoved = j;}
